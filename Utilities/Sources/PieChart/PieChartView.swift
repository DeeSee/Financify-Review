/// ## Ревью: `Utilities/Sources/PieChart/PieChartView.swift`
/// 
/// ## Важно
/// - **Нет поддержки Dynamic Type/доступности**:
///   - Текст легенды рисуется вручную через `NSString.draw`, фиксированный размер шрифта.
///   - VoiceOver не увидит элементы легенды/сегменты как доступные элементы.
/// - Строка `"Остальные"` захардкожена (локализация).
/// 
/// ## Нюансы
/// - `prepareSegments` предполагает, что значения неотрицательные. Если вдруг придут отрицательные значения, проценты/рисование будут некорректными.
/// - Палитра фиксирована на 6 цветов; при большом количестве категорий цвета начнут повторяться (частично решается “Остальные”).
/// 
/// ## Предложения
/// - Для accessibility: либо добавлять `UIAccessibilityElement` для сегментов/легенды, либо реализовать легенду как реальные UILabel/stack views.
/// - Локализовать `"Остальные"`.
/// - Явно нормализовать значения (например, игнорировать отрицательные или брать `abs`, в зависимости от смысла).
/// 
