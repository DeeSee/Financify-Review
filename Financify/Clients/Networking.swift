/// ## Ревью: `Financify/Clients/Networking.swift`
/// 
/// ## Критично
/// - **Тело ответа при ошибках практически всегда теряется**: в `AFError.underlyingData` сейчас возвращается `nil` во всех ветках, поэтому `NetworkError.serverError(statusCode:data:)` почти всегда приходит с `data == nil`. В результате отладочные принты “Тело ответа сервера” в UI-слое не работают.
/// - **`requestStatus` достаёт statusCode через `dataRequest.response?.statusCode`** после `serializingData().value`. Это не самый надёжный путь: корректнее брать `HTTPURLResponse` из результата/таска сериализации (чтобы не зависеть от сайд-эффектов `DataRequest`).
/// 
/// ## Важно
/// - **`APIEndpoint.baseURL` захардкожен**. Для приложения это обычно конфигурация окружения (dev/stage/prod) и должна быть вынесена в конфиг (`xcconfig`/plist) + легко подменяться в тестах.
/// - **Извлечение `API_KEY` идёт из `Info.plist`** (а значение попадает туда через подстановку `$(API_KEY)`), поэтому важно документировать настройку `.xcconfig → Info.plist → Bundle`.
/// - **`.validate()`**:
///   - Для “обычных” запросов ок.
///   - Для `requestStatus` (где цель — получить код) может быть лучше **не валидировать**, а возвращать статус и (опционально) тело, чтобы callers сами решали, что фатально.
/// 
/// ## Нюансы / стиль
/// - **encode/decode через `DispatchQueue.global + withCheckedThrowingContinuation`**: код явно выносит JSON (де)кодирование на global queue, но это добавляет сложности (в т.ч. с отменой задач). Альтернатива — вызывать `NetworkClient` вне MainActor и/или централизовать выполнение (например, через отдельный executor), не усложняя encode/decode continuations.
/// - **Единый `JSONDecoder/JSONEncoder`**: сейчас дефолтные создаются в параметрах; для настройки (дат, стратегий) лучше иметь централизованные фабрики.
/// 
/// ## Предложения
/// - **Пробросить `Data` при ошибках**: извлекать response data из результата Alamofire `serializingData()` и прокидывать в `NetworkError.serverError`.
/// - **Сделать `baseURL` конфигурируемым** (init параметр/Environment).
/// - **Добавить типизированные ошибки** для 401/403/429 и единую точку обработки токена/авторизации.
/// 
