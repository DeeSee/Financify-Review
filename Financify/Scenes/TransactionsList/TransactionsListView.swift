/// ## Ревью: `Financify/Scenes/TransactionsList/TransactionsListView.swift`
/// 
/// ## Важно
/// - **Много точек, где вызывается `refresh()`**:
///   - `.task { refresh }`
///   - `selectedSortOption` внутри VM
///   - `onDismiss` у fullScreenCover
///   - восстановление сети
///   Это повышает риск гонок/лишних запросов и “мигания”.
/// - **UI‑строки захардкожены** (например, “Синхронизация...”, заголовки секций).
/// 
/// ## Нюансы
/// - Логика плейсхолдеров через два `.if(...)` подряд можно упростить (читаемость).
/// - Плюс‑кнопка отображается всегда, даже offline. Это ок для offline‑first, но тогда важно, чтобы UX/тексты объясняли, что операция уйдёт в очередь.
/// 
/// ## Предложения
/// - Перенести локализацию строк в `Localizable.xcstrings`.
/// - Рассмотреть, чтобы VM публиковала “effective state” (loading/empty/error) и View меньше решала сама.
/// 
