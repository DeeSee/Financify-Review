/// ## Ревью: `Financify/Scenes/TransactionEditor/TransactionEditorViewModel.swift`
/// 
/// ## Критично
/// - **`isLoading` не `@Published`**: ViewModel меняет `isLoading`, но UI об этом не узнает. Сейчас UI и не использует этот флаг, что наводит на мысль о незавершённой логике.
/// - **Парсинг суммы при редактировании существующей транзакции может ломаться**:
///   - `loadInitialData()` выставляет `amountText = tx.amount.moneyFormatted`.
///   - `moneyFormatted` добавляет пробелы‑разделители тысяч.
///   - `amountDecimal()` использует `NumberFormatter`, но не задаёт `groupingSeparator`/`usesGroupingSeparator`, из‑за чего строка типа `"1 234"` распарсится в `nil` → `.zero`.
///   - В итоге `canSave` и валидация `save()` могут ошибочно считать сумму = 0 и блокировать сохранение.
/// 
/// ## Важно
/// - Повторяющаяся обработка `NetworkError` с `print` (как и в других VM) — лучше централизовать.
/// - `Currency(jsonTitle:)` может `fatalError` на неизвестном коде валюты.
/// 
/// ## Предложения
/// - Либо хранить `amountText` как “сырой” числовой ввод без пробелов, либо настроить `NumberFormatter` на парсинг группировки.
/// - Использовать `canSave` для disable кнопки “Сохранить/Создать”, чтобы не показывать алерт “заполните” после клика.
/// - Вынести обработку ошибок и сообщения в единый слой.
/// 
