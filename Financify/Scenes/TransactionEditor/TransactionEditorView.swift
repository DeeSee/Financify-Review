/// ## Ревью: `Financify/Scenes/TransactionEditor/TransactionEditorView.swift`
/// 
/// ## Критично
/// - **Текст кнопки удаления всегда “Удалить расход”**, даже на экране доходов. Это явный UX/semantic баг (для дохода должно быть “Удалить доход” или нейтральное “Удалить операцию”).
/// 
/// ## Важно
/// - Кнопка “Создать/Сохранить” всегда доступна, хотя в VM есть `canSave`. Сейчас UX такой: пользователь нажимает → получает алерт. Обычно лучше заранее disable кнопку.
/// - Много строк и констант захардкожено (локализация).
/// 
/// ## Нюансы
/// - В селекторе категории в label показывается только имя, без emoji (хотя в списке emoji есть). Может быть намеренно, но стоит проверить UX.
/// 
