/// ## Ревью: `Financify/Scenes/Balance/Additional/ChartInteractionOverlay.swift`
/// 
/// ## Нюансы
/// - `updateSelection` линейно проходит по всем `chartData` на каждый drag. Для 30/24 точек это ок, но при расширении периодов может стать заметно.
/// - Комбинация `DragGesture(minimumDistance: 0)` + `LongPressGesture` через `simultaneously(with:)` иногда ведёт к неочевидной UX‑семантике (что отменяет что). Сейчас выглядит рабоче.
/// 
/// ## Предложения
/// - Если точек станет много — искать ближайшую по дате более эффективно (например, бинарным поиском по отсортированному массиву).
/// - Убедиться, что жесты корректно работают с VoiceOver/доступностью (не блокируют скролл и т.п.).
/// 
