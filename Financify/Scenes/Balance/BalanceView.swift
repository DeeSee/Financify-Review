/// ## Ревью: `Financify/Scenes/Balance/BalanceView.swift`
/// 
/// ## Критично
/// - **Редактирование баланса использует `.decimalPad`, но дробная часть невозможна**:
///   - `commitTotalEdit()` фильтрует только цифры (`isWholeNumber`), игнорируя десятичный разделитель.
///   - Переменная названа `digitsAndDot`, но “dot” не учитывается.
/// - **Ошибка формата валюты в неиспользуемом коде**: функция `chartSelectionPopover(...)` объявлена, но нигде не вызывается; внутри неё используется `.currency(code: viewModel.selectedCurrency.rawValue)`, где `rawValue` — символ (`₽/$/€`), а не ISO‑код (`RUB/USD/EUR`). Это сигнал о путанице “код vs символ”.
/// 
/// ## Важно
/// - Очень много UI‑констант и строк прямо в файле — усложняет локализацию и поддержку.
/// - `chartDateLabels` форс‑анврапится (`!`) — безопасно по текущим условиям, но лучше избегать, чтобы не ловить крэши при будущих изменениях.
/// 
/// ## Предложения
/// - Сейчас редактор фактически поддерживает только целые (десятичный разделитель выкидывается); поэтому `.numberPad` будет честнее. Для поддержки копеек нужно переписать парсинг/валидацию ввода.
/// - Выровнять работу с валютой: хранить ISO‑код, а символ — отдельно.
/// - Вынести строки и часть констант в локализацию/отдельные типы.
/// 
