/// ## Ревью: `Financify/Scenes/History/HistoryViewModel.swift`
/// 
/// ## Критично
/// - **`fromDate` и `toDate` могут триггерить `refresh()` дважды**:
///   - В `willSet` одного поля вы меняете другое поле, у которого есть `didSet { Task { refresh() } }`.
///   - Затем срабатывает `didSet` исходного поля → второй refresh.
///   - Это лишние запросы и риск гонок результатов.
/// - **Потенциальный крэш на неизвестной валюте** (`Currency(jsonTitle:)` → `fatalError`).
/// 
/// ## Важно
/// - Нет отмены/дедупликации refresh’ей, хотя вызовов много (смена дат, сортировки, восстановление сети, `.task`).
/// - Ошибки в основном уходят в `print`, без понятного состояния UI (пользователь видит пусто).
/// 
/// ## Предложения
/// - Использовать один источник правды для периодов и один “debounced” refresh:
///   - например, `@Published var dateRange` и `.task(id:)`.
/// - Добавить cancellation предыдущего refresh.
/// - Показывать ошибку пользователю (alert/empty state).
/// 
