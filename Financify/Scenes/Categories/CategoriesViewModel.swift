/// ## Ревью: `Financify/Scenes/Categories/CategoriesViewModel.swift`
/// 
/// ## Критично
/// - **Fuzzy‑поиск потенциально тяжёлый**:
///   - `fuzzyMatch` перебирает окна по всей строке и считает Damerau‑Levenshtein для множества подстрок.
///   - Вызывается на каждое изменение `searchText` (в computed `filteredCategories`/`suggestions`) без debounce → при большом списке категорий может лагать UI.
/// 
/// ## Важно
/// - Ошибки загрузки категорий только `print`‑ятся. Пользователь не получает понятного состояния “ошибка/нет сети”.
/// 
/// ## Нюансы
/// - `suggestions` может содержать дубликаты, если есть одинаковые имена категорий.
/// 
/// ## Предложения
/// - Добавить debounce (например, через `Task.sleep` + cancellation) или вынести поиск в отдельный async pipeline.
/// - Оптимизировать метрику (например, искать по префиксу/contains перед дорогим расстоянием).
/// - Показать пользователю ошибку (alert/empty state).
/// 
