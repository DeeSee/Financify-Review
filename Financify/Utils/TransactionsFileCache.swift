/// ## Ревью: `Financify/Utils/TransactionsFileCache.swift`
/// 
/// ## Критично
/// - **Класс нигде не используется в проекте** (по текущему коду). Это либо “забытый” модуль, либо недоделанная фича импорта/экспорта. В текущем виде это технический долг.
/// 
/// ## Важно
/// - **`NumberFormatter/CSV` и формат данных**:
///   - `Transaction.parse(csvLine:)` использует наивный split по запятой. CSV с запятыми внутри кавычек будет парситься неверно.
///   - ISO8601 форматтер без fractional seconds может не совпасть с форматом API/экспорта.
/// - **`Task.detached` внутри actor**:
///   - Идея вынести IO в фон понятна, но `Task.detached` сложнее в отмене и трассировке.
///   - Альтернатива: `withCheckedThrowingContinuation` + `DispatchQueue` или `Task(priority:)` без detaching, если нет необходимости отрывать контекст.
/// - **Старая закомментированная тестовая data** внутри `transactions` — лучше удалить, чтобы не мешало поддержке.
/// 
/// ## Предложения
/// - Либо удалить файл/модуль, либо интегрировать и покрыть тестами.
/// - Для CSV‑импорта нужен нормальный CSV‑парсер или корректный state machine для кавычек/экранирования.
/// 
