/// ## Ревью: `Financify/Extensions/UIView+Pin.swift`
/// 
/// ## Важно
/// - **Нейминг `ConstraintMode`**:
///   - `.grOE` / `.lsOE` выглядят неочевидно. Лучше `greaterOrEqual` / `lessOrEqual`.
/// - Комментарий “Will throw exceptions…” верный по смыслу: AutoLayout действительно может упасть, если якоря из разных иерархий, но код никак это не проверяет (и не может надёжно проверить).
/// 
/// ## Нюансы
/// - Методы `pinRight/pinBottom` умножают константу на `-1`. Это удобно (передаём положительный inset), но может путать при чтении и особенно при передаче отрицательных значений.
/// - В каждом методе принудительно выставляется `translatesAutoresizingMaskIntoConstraints = false`. Это нормальный helper‑паттерн, но важно помнить, что это сайд‑эффект на view.
/// 
/// ## Предложения
/// - Переименовать режимы и/или добавить doc‑комментарии с примерами.
/// - Для сложных экранов рассмотреть использование `NSLayoutConstraint.activate([...])` с явными constraints, чтобы отладка была проще.
/// 
