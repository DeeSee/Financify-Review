/// ## Ревью: `Financify/Extensions/Color+Hex.swift`
/// 
/// ## Что хорошо
/// - Есть поддержка `3/6/8`‑значных HEX и (опционально) альфы.
/// - При ошибке парсинга возвращается `.clear`, а не “молчаливый” чёрный.
/// 
/// ## Нюансы
/// - Логика дублируется с `UIColor(hex:)`, но поведение разное (там нет валидации и alpha).
/// 
/// ## Предложения
/// - Унифицировать подход для `Color` и `UIColor` (в идеале один общий парсер).
/// 
