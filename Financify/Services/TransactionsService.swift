/// ## Ревью: `Financify/Services/TransactionsService.swift`
/// 
/// ## Критично
/// - **`updateLocalStore(with:)` делает wipe всей таблицы транзакций** (`delete(model: PersistentTransaction.self)`), затем вставляет только то, что пришло с сервера.
///   - Это ломает “offline-first” семантику: если в очереди `PendingOperation` есть несинхронизированные изменения (POST/PUT/DELETE), то при успешном онлайн‑fetch они **не накладываются** поверх данных сервера и могут визуально/фактически пропасть из локального стора.
///   - При временных ошибках синка (5xx) ситуация особенно опасна: операция остаётся в очереди, но локальная транзакция может быть удалена wipe-ом.
/// - **Офлайн POST использует временный `id` на основе секунд**: `-Int(Date().timeIntervalSince1970)`.
///   - При нескольких добавлениях в одну секунду возможны коллизии id → конфликт `.unique` в SwiftData и/или перетирания.
/// - **Попытка прокинуть `tempId` через `JSONEncoder.userInfo` не работает**:
///   - `TransactionRequest` не читает `userInfo`, поэтому payload не содержит temp id.
///   - `SynchronizationService` выполняет POST без чтения ответа → reconciliation невозможен.
/// 
/// ## Важно
/// - **Fallback-логика накладывает pending PUT поверх локальных данных только при сетевой ошибке**, но **не делает этого при успешном fetch**. Для пользователя это выглядит как “пропала правка/удаление”.
/// - **`backupService` используется внутри actor’а, но сам сервис не actor**: см. риски конкурентности SwiftData.
/// - **Логирование через `print`**: шум в проде, нет уровней/категорий.
/// 
/// ## Нюансы
/// - `parseId(from:)` берёт последний path component. Это ок для `/transactions/{id}`, но лучше ограничить на уровне добавления операций, чтобы не парсить “случайные” пути.
/// 
/// ## Предложения
/// - Вместо wipe-and-replace: **upsert** по `id` + сохранение локальных “pending” флагов.
/// - После любого источника данных (онлайн/офлайн) формировать “effective view” как:
///   - `remoteOrLocalBase` + apply `PendingOperation` (в timestamp порядке).
/// - Для офлайн POST: хранить отдельный `localUUID` или стабильный temp id с меньшим риском коллизий; после успешного POST — обновлять локальный объект реальным server id.
/// 
