/// ## Ревью: `Financify/Services/BankAccountService.swift`
/// 
/// ## Критично
/// - **Выбор “primary” аккаунта как `accounts.first`**:
///   - Если сервер возвращает несколько счетов, порядок может быть не гарантирован → пользователь может работать не с тем счётом.
///   - Нужен явный признак primary либо выбор/настройка в UI.
/// - **`backupService` не actor, но используется из actor’а** (и одновременно из других акторов) → риск гонок `ModelContext` (см. `BackupService`).
/// 
/// ## Важно
/// - **Расчёт офлайн-баланса** выглядит продуманно (накладывание pending операций), но:
///   - Он зависит от наличия категорий в локальном сторе (`fetchCategory`). Если категории отсутствуют/не синхронизированы, изменения могут частично не примениться → баланс будет неверным.
///   - Логика для операций `PUT`/`DELETE` по транзакциям довольно сложная; полезно иметь тесты на последовательности операций (POST→PUT→DELETE и т.п.).
/// - **Обновление баланса/валюты берёт первый локальный аккаунт** (`FetchDescriptor<PersistentBankAccount>().first`). Это снова привязка к “первому” без явного primary id.
/// 
/// ## Нюансы
/// - `AccountUpdateRequest` кодируется в строки для `balance`, но в доменной модели `BankAccount.encode` используется `String(describing: balance)` (другая стратегия) — стоит унифицировать форматы чисел.
/// 
/// ## Предложения
/// - Явно хранить/выбирать `primaryAccountId` (в настройках/в storage).
/// - Добавить unit-тесты на офлайн-расчёт баланса:
///   - POST income/expense
///   - PUT меняет категорию income↔expense
///   - DELETE откатывает изменения
/// - Повысить устойчивость к отсутствующим категориям: либо кэшировать `isIncome` в payload операции, либо гарантировать синхронизацию категорий до расчёта.
/// 
